...
void show_char(const char letter, unsigned int pos, bool upper_line=true){
	...
	if(upper_line){
		...
		switch (letter) {
			...
			case 'w' : *addr=0b00110011;break;
			case 'x' : *addr=0b01000100;break;
			case 'y' : *addr=0b11001011;break;
...
void LCD::show_number(long int number, bool upper_line) {
  if ((upper_line && number <= 9999) || (!upper_line && number <= 99999)) {
	  int pos = 1;
